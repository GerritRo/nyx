...
class HEALPixIndex:
    ...
    def nearest(self, ra: float, dec: float, radius: float) -> tuple[int, float]:
        ...
        separation : float
            Separation in degrees.  On a miss, the distance to the closest
            row found in a widened search, or ``inf`` if there is none.
        """
        # ``query`` is only a candidate filter: its disc is inclusive and its
        # pixels are far coarser than a match radius, so a row it returns can
        # still be well outside *radius*.  The separation decides.
