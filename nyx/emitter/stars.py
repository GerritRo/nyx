...
class Stars(CatalogEmitter):
    ...
    sky_map : numpy.ndarray, shape (n_cond, npix)
        Complete sky in linear flux, nested ordering.  This is what a
        ``Stars`` fundamentally is; the resolved stars below only refine it.
    bright_ra, bright_dec : numpy.ndarray or None
        Positions of the resolved stars, in degrees.  ``None`` leaves the
        catalog purely diffuse, which is what :func:`gaia_star_field` wants
        for the half it has already taken the bright stars out of.
    ...
    brightness : array-like or None
        ...
        for both the map and the resolved stars; see
        :class:`~nyx.emitter.base.Emitter`.
    ...
    def _pop_conditions(self, idx: int) -> np.ndarray:
        ...
        point-source list without leaving the sky.  In-scattering stays off
        by default, because this catalog's diffuse map covers the whole sky
        and the star's scattered halo is already in the map path.
        """
    ...
    def _prepare(self, obs) -> SourceObsData:
        ...
        # it leaves the resolved list -- but it stays in the flux subtracted
        # from the quasi-point pixel below, or its light would come back
        # through that pixel on top of the PointSource.
        ...
        # Resolved stars lead the quasi-point map pixels, matching the order
        # ``source_conditions`` was stacked in above.
        ...
        return SourceObsData(
            ...
            source_coords=jnp.asarray(source_coords),
            direct=False,
            per_obs_fields=("diffuse_conditions", "source_coords"),
        )
...
class BrightStars(CatalogEmitter):
    ...
    individually.  The source count is fixed across observations: stars below
    the horizon are switched off by the ``active`` condition rather than
    filtered out.
    ...
    #: A lone star from here carries its own halo: this catalog has no
    #: diffuse map for the scattered light to already live in.
    _pop_inscatter = True

    def _pop_conditions(self, idx: int) -> np.ndarray:
        """This catalog's photometry for the star, with ``active=1`` appended.

        A star popped from here keeps its proper motion but loses the horizon
        mask: the mask belongs to this catalog's fixed-count render, not to a
        source prepared on its own, so a popped star below the horizon still
        contributes a heavily extincted point.
        """
    ...
    def _prepare(self, obs) -> SourceObsData:
        ...
        return SourceObsData(
            source_conditions=jnp.asarray(conditions),
            source_coords=jnp.asarray(track),
            inscatter=True,
    ...
    def from_anderson2012(cls, geo, **kwargs) -> BrightStars:
        ...
        (1998) library.  Needs network access on first use, for the Johnson
        passbands.
...
def gaia_star_field(geo, lim_mag: float = 7.0):
    ...
    # The colour grid spans the whole catalog, so both halves interpolate the
    # same spectral library over the same axis.  It is built once and wrapped
    # twice; only the point half reads the horizon mask.
