...
def _jacobian_columns(jvp_fn: Callable[[Any], Any], n: int, m: int, chunk: int) -> jax.Array:
    """Build ``J^T`` by using basis vectors.
...
class _Decomposition:
    """The eigendecomposition of ``J^T J``."""
...
def _decompose(
    ...
    *,
    ...
    def residuals_of_flat(x: jax.Array) -> jax.Array:
        return fu.ravel_pytree(residuals_fn(eqx.combine(unravel(x), static, is_leaf=is_parameter)))[
            0
        ]
    ...
    if n_null > 0:
        ...
        directions = _null_space_parameters(diff, eigvecs[:, ~keep])
        warnings.warn(
            ...
            f"{cond:.3e}).  The "
            f"unconstrained direction(s) lie mostly along {directions}.",
            NyxWarning,
            stacklevel=3,
        )
...
def parameter_errors[T](
    ...
    *,
    ...
    batch_size : int, optional
        Jacobian columns per kernel launch.
    ...
    # Only the diagonal of the covariance is needed
...
def rescale_from_errors[T](fitted: T, errs: T) -> T:
    """Rescale every trainable Parameter using its error estimate.
