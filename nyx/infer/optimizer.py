...
def _result_text(sol: optx.Solution[Any, Any]) -> str:
    ...
    str
    """
    message = str(optx.RESULTS[sol.result]).strip()  # type: ignore[misc]
...
class Optimizer:
    ...
    solver : optimistix.AbstractIterativeSolver
        E.g. ``optx.LBFGS`` or ``optx.LevenbergMarquardt``.
    ...
    def __init__(self, fn: Callable[[Any], Any], solver: Any) -> None:
        ...
        self._fn_user = fn
        self._fn_is_scalar: bool | None = None
        self._inner_by_kind: dict[bool, Callable[..., Any]] = {}

    def _check_fn(self, model: Any) -> bool:
        """Probe *fn*'s output shape for *model*.
        ...
        -------
        bool
            Whether *fn* returns a scalar for *model*.
    ...
    def _make_inner(self, model: Any) -> tuple[Callable[..., Any], Any, Any]:
        ...
        fn_is_scalar = self._check_fn(model)
        inner = self._inner_by_kind.get(fn_is_scalar)
        if inner is None:
            fn_user = self._fn_user
            user_fn: Callable[[Any], Any]
            if not self._is_ls and not fn_is_scalar:
            ...
            self._inner_by_kind[fn_is_scalar] = inner
        ...
        *,
        ...
        for index in np.ndindex(*chi2.shape):
            ...
            if callback is not None:
                callback(index, point, float(chi2[index]))
