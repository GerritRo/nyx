...
def walk(
    ...
    *,
    ...
    def recurse(child: Any, step: Any) -> Iterator[tuple[tuple[Any, ...], Any]]:
        yield from walk(
            child,
            match,
            prune=prune,
            descend_into_match=descend_into_match,
            _prefix=_prefix + (step,),
        )

    if isinstance(tree, eqx.Module):
        for name in tree.__dataclass_fields__:
            yield from recurse(getattr(tree, name), name)
    elif isinstance(tree, dict):
        for key, child in tree.items():
            yield from recurse(child, key)
    elif isinstance(tree, (list, tuple)):
        for i, child in enumerate(tree):
            yield from recurse(child, i)
...
def friendly_path(parts: Iterable[Any]) -> str:
    """Join path *parts* into a dotted name.
...
def matching_paths(tree: Any, pattern: str) -> list[tuple[Any, ...]]:
    ...
    if not found:
        ...
        raise KeyError(
            f"{pattern!r} matches no parameter. "
...
def _wrap_value(path: str, target: Any, value: Any) -> Any:
    ...
    """
    if is_parameter(target) and not is_parameter(value) and jnp.ndim(value) == 0:
        # Broadcast scalar parameter
        value = jnp.broadcast_to(jnp.asarray(value), jnp.shape(target.factor))
