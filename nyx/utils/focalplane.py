...
def _hex_lattice(xy: np.ndarray) -> tuple[float, float]:
    """Pitch and orientation of the hexagonal lattice the centres *xy* lie on.

    The pitch is the median distance between neighbouring pixels, the
    angle the direction of the lattice rows, folded into ``[-pi/6, pi/6)``
    by the lattice's six-fold symmetry.
    """
...
def camera_image(
    ...
    **kwargs
    ...
    The returned collection stays on *ax*; call ``set_array`` on it to show
