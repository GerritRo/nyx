...
def gaussian_blur(field, sigma_nodes):
    """Blur a lattice array by a separable Gaussian, widths in lattice nodes.

    Applied as the Fourier multiplier ``exp(-2 pi^2 sigma^2 f^2)``.

    Parameters
    ----------
    field : jax.Array, shape (n_rows, n_cols)
        Values on the lattice; axis 0 is longitude.
    sigma_nodes : jax.Array, shape (2,)
        Gaussian width along each axis, in lattice nodes.

    Returns
    -------
    jax.Array, shape (n_rows, n_cols)
    """
    n_rows, n_cols = field.shape
    f_lon = jnp.fft.fftfreq(n_rows)[:, None]
    f_lat = jnp.fft.rfftfreq(n_cols)[None, :]
    attenuation = jnp.exp(
        -2.0 * jnp.pi**2 * (sigma_nodes[0] ** 2 * f_lon**2 + sigma_nodes[1] ** 2 * f_lat**2)
    )
    return jnp.fft.irfft2(jnp.fft.rfft2(field) * attenuation, s=(n_rows, n_cols))


def project_lattice(lattice: PixelLattice, values, coords, rates, blur=None):
    ...
    blur : jax.Array, shape (2,), or None
        Gaussian widths in lattice nodes to blur the rasterised sources by
        before projecting.
    ...
    if blur is not None:
        raster = gaussian_blur(raster.reshape(n_rows, n_cols), blur).reshape(-1)
...
def response_centroid(lattice: PixelLattice, values):
    """Where each pixel looks via the first moment of its response.
    ...
    # Fall back to the window centre if pixel is dark.
